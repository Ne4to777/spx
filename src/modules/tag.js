...
const getTermStore = clientContext => SP.Taxonomy.TaxonomySession
  ...
  return {
    ...
    create: async opts => {
      const { clientContexts, result } = await iteratorInstanced(({ clientContext, element }) => {
        ...
        const spObject = getTermSet(clientContext).createTerm(elementUrl, 1033, getInstanceEmpty(SP.Guid.newGuid).toString())
        return load(clientContext)(spObject)(opts);
      })
