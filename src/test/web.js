...
import {
	assertObject, assertCollection, testIsOk, assert, map, identity
} from '../lib/utility'
...
const crud = async () => {
	await site('test/spx/createdWebSingle')
	...
	const updatedWeb = await assertObjectProps('updated web')(
		site({ Url: 'test/spx/createdWebSingle', Description: 'Test description' }).update()
	)
...
const crudCollection = async () => {
	await site(['test/spx/createdWebMulti', 'test/spx/createdWebMultiAnother'])
		.delete({ noRecycle: true, silentErrors: true })
		.catch(identity)
	const newWebs = await assertCollectionProps('new webs')(
		site(['test/spx/createdWebMulti', 'test/spx/createdWebMultiAnother']).create()
	)
