...
const crud = async _ => {
  ...
  await site.tag(tagName).delete({ silentErrors: true }).catch(identity)
...
const crudCollection = async _ => {
  ...
  await site.tag([tagName, tagNameAnother]).delete({ silentErrors: true }).catch(identity)
  const newTags = await assertCollectionProps('new tags')(site.tag([{ Url: tagName }, { Url: tagNameAnother }]).create());
  assert(`Name is not a "${tagName}"`)(newTags[0].Name === tagName);
  assert(`Name is not a "${tagNameAnother}"`)(newTags[1].Name === tagNameAnother);
  const updatedTags = await assertCollectionProps('updated tag')(site.tag([{ Url: tagName, Name: newTagName }, { Url: tagNameAnother, Name: newTagNameAnother }]).update().catch(_ =>
    site.tag([newTagName, newTagNameAnother]).delete()));
...
export default async _ => Promise.all([
